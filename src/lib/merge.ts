import type { AgentBinding, AgentConfig, BotConfigPlan, OpenClawConfig } from "./types.js";

function clone<T>(value: T): T {
  return structuredClone(value);
}

function mergeAgents(existingAgents: AgentConfig[], plannedAgents: AgentConfig[]): AgentConfig[] {
...
function mergeBindings(existingBindings: AgentBinding[], plannedBindings: AgentBinding[]): AgentBinding[] {
  const managedKeys = new Map<string, AgentBinding>(
    plannedBindings.map((binding) => [`${binding.match.channel}::${binding.match.accountId}`, clone(binding)] as const)
  );
  const merged = existingBindings.map((binding) => {
    const key = `${binding.match?.channel}::${binding.match?.accountId}` as string;
...
export function mergeConfig(currentConfig: OpenClawConfig, plan: BotConfigPlan): OpenClawConfig {
    ...
    for (const [channel, patchChannel] of Object.entries(plan.patch.channels)) {
      ...
      merged.channels[channel] = {
        ...
        accounts: {
          ...existingAccounts,
          ...Object.fromEntries(
            Object.entries(patchAccounts).map(([accountId, accountConfig]) => [
              accountId,
              {
