...
import type { BotConfigPlan, OpenClawConfig } from "./types.js";
...
export async function createBackup(configPath: string, backupDir?: string): Promise<string> {
...
export async function applyPlan(plan: BotConfigPlan, configPath: string, backupPath?: string) {
...
export async function rollbackConfig(configPath: string, backupPath: string) {
  const rawBackup = await fs.readFile(backupPath, "utf8");
  const parsedBackup = JSON.parse(rawBackup) as OpenClawConfig;
