...
本文说明如何在本地安装和使用 `openclaw-multi-bot-config` 这个 Skill。

当前仓库中的相关产物位置：

- Skill 源目录：`D:\work\code\moltbot-china\openclaw-multi-bot-config`
- 可分发包：`D:\work\code\moltbot-china\openclaw-multi-bot-config\.artifacts\dist\openclaw-multi-bot-config.skill`
...
对 `openclaw-multi-bot-config` 来说，推荐优先用共享安装：

- 目录：`C:\Users\Administrator\.openclaw\skills\openclaw-multi-bot-config`
...
Copy-Item -Path 'D:\work\code\moltbot-china\openclaw-multi-bot-config' `
  -Destination 'C:\Users\Administrator\.openclaw\skills\openclaw-multi-bot-config' `
  -Recurse -Force
```
...
```powershell
openclaw skills info openclaw-multi-bot-config
...
- `openclaw skills info openclaw-multi-bot-config` 显示 `Ready`
- `Source` 显示为 `openclaw-managed` 或 `openclaw-workspace`
- `openclaw skills check` 在 `Ready to use` 中列出 `openclaw-multi-bot-config`
...
- `D:\work\code\moltbot-china\openclaw-multi-bot-config\.artifacts\dist\openclaw-multi-bot-config.skill`
...
```powershell
New-Item -ItemType Directory -Force -Path 'C:\Users\Administrator\.openclaw\skills' | Out-Null
python -c "import zipfile; zipfile.ZipFile(r'D:\work\code\moltbot-china\openclaw-multi-bot-config\.artifacts\dist\openclaw-multi-bot-config.skill').extractall(r'C:\Users\Administrator\.openclaw\skills')"
```

解压后目录应为：

```text
C:\Users\Administrator\.openclaw\skills\openclaw-multi-bot-config\
```
...
- 已复制到 `C:\Users\Administrator\.openclaw\skills\openclaw-multi-bot-config`
- `openclaw skills info openclaw-multi-bot-config` 显示 `Ready`
...
- 运行时目录：`D:\work\code\moltbot-china\openclaw-multi-bot-config\.artifacts\runtime\openclaw-multi-bot-config`
...
1. 把 `openclaw-multi-bot-config` 目录复制到 `C:\Users\Administrator\.openclaw\skills\openclaw-multi-bot-config`
2. 用 `openclaw skills info openclaw-multi-bot-config` 确认它是 `Ready`
