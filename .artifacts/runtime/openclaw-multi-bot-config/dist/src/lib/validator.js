...
function validateDefaultAccount(targetChannel, channelName) {
    const issues = [];
    if (!targetChannel.defaultAccount) {
        return issues;
    }
    if (!targetChannel.accounts?.[targetChannel.defaultAccount]) {
        issues.push(issue("PLAN_INVALID", `Default account '${targetChannel.defaultAccount}' does not exist for channel '${channelName}'`, "error", `channels.${channelName}.defaultAccount`));
    }
    return issues;
}
export async function validatePlan(plan, currentConfig) {
    ...
    for (const resolvedTarget of plan.resolved.targets) {
        ...
        for (const account of resolvedTarget.accounts) {
            if (accountIds.has(account.accountId)) {
                issues.push(issue("PLAN_INVALID", `Duplicate accountId '${account.accountId}' in channel '${resolvedTarget.channel}'`, "error", `targets.${resolvedTarget.channel}.accounts.${account.accountId}`));
            }
    ...
    for (const existingAgent of previewConfig.agents?.list ?? []) {
        ...
        if (owner && owner !== existingAgent.id && !plan.resolved.options.allowAgentWorkspaceReuse) {
            issues.push(issue("PLAN_CONFLICT", `Workspace '${existingAgent.workspace}' is shared by '${owner}' and '${existingAgent.id}'`, "error", `agents.${existingAgent.id}.workspace`));
        }
        workspaceOwners.set(existingAgent.workspace, existingAgent.id);
    }
    const currentBindings = new Map((currentConfig.bindings ?? []).map((binding) => [`${binding.match?.channel}::${binding.match?.accountId}`, binding.agentId]));
    for (const binding of plan.resolved.bindings) {
        ...
        if (existingAgentId && existingAgentId !== binding.agentId && !plan.resolved.options.allowBindingOverride) {
            issues.push(issue("PLAN_CONFLICT", `Binding conflict for ${binding.match.channel}.${binding.match.accountId}: existing agent '${existingAgentId}', planned agent '${binding.agentId}'`, "error", `bindings.${binding.match.channel}.${binding.match.accountId}`));
        }
    }
    if (plan.resolved.operation === "modify-existing-channel" &&
