    ...
    targets.forEach((target, targetIndex) => {
        ...
        target.accounts.forEach((account, accountIndex) => {
            if (account.accountId.length === 0) {
                issues.push(issue("INVALID_REQUEST", "Account.accountId is required", "error", `targets[${targetIndex}].accounts[${accountIndex}].accountId`));
            }
            if (accountSet.has(account.accountId)) {
                issues.push(issue("INVALID_REQUEST", `Duplicate accountId in target ${target.channel}: ${account.accountId}`, "error", `targets[${targetIndex}].accounts[${accountIndex}].accountId`));
            }
            accountSet.add(account.accountId);
            if (!isObject(account.credentials) || Object.keys(account.credentials).length === 0) {
                issues.push(issue("INVALID_REQUEST", "Account.credentials must be a non-empty object", "error", `targets[${targetIndex}].accounts[${accountIndex}].credentials`));
            }
        });
        if (target.defaultAccount && !target.accounts.some((account) => account.accountId === target.defaultAccount)) {
            issues.push(issue("INVALID_REQUEST", `Target.defaultAccount must reference an account in the same target: ${target.defaultAccount}`, "error", `targets[${targetIndex}].defaultAccount`));
        }
    });
    return issues;
}
...
export async function loadRequest(requestPath) {
    const raw = await readJsonFile(requestPath, "INVALID_REQUEST");
    ...
    const sessionPolicy = isObject(raw.sessionPolicy)
        ? { ...DEFAULT_SESSION_POLICY, ...raw.sessionPolicy }
        : DEFAULT_SESSION_POLICY;
    const options = isObject(raw.options)
        ? { ...DEFAULT_OPTIONS, ...raw.options }
        : DEFAULT_OPTIONS;
    const request = {
        version: "1",
        action: raw.action,
